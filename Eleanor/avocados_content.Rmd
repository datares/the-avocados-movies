...
moviescontent <- ggplot(amtofmovies, aes(x=service, y=numofmovies, color=service)) + geom_bar(stat="identity", fill="#CECCE4")
moviescontent+scale_color_manual(values=c("#7BAA40", "#113CCF", "#1ce783", "#E50914"))

## Shows

ggplot(data = amtofshows, aes(x=service, y=numofshows)) + geom_bar(stat = "identity")
showscontent <- ggplot(amtofshows, aes(x=service, y=numofshows, fill=service)) + geom_bar(stat="identity", color="#CECCE4")
showscontent+scale_fill_manual(values=c("#7BAA40", "#113CCF", "#1ce783", "#E50914"))
```
